...
2. Idempotency - Each migration should only be run once against an environment. Thus, do not manually change the migration afterwards but rather create a new one
...
5. Commit as batches, i.e. `let batch = db.batch()`. Batches should not change more than 500 documents at once. If so, please split it into multiple batches